    ...
    <div>
      <link rel="stylesheet" href={join(relative(ROOT, __dirname), 'recoverybar.css')} />
      {
        switch @props.decksAddition.state[@props.deckIndex]
          ...
          when 1 # akashi
          when 2 # cond
            <ProgressBar key={1} className="rec-progress-#{@props.deckIndex} cond" now={100} />
          when 5 # repairing
            <ProgressBar key={1} className="rec-progress-#{@props.deckIndex} repair" now={100} />
          when 6 # in mission
            <ProgressBar key={1} className="rec-progress-#{@props.deckIndex} mission" now={100} />
      }
    </div>
